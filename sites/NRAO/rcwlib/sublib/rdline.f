      SUBROUTINE RDLINE( WORD, WLEN, NWORDS, FIN, TOUT )
C
...
C     1995Feb15  Ignore *************
C
C     2009Feb09  INLINE lengthened to 2000 (DiFX large numbers of IFs)
C
      INTEGER        MAXCH
      CHARACTER      INLINE*2000
      CHARACTER      WORD(*)*(*)
...
C
     ...
     3           INLINE(CHAR1:CHAR1+9) .NE. '++++++++++' ) THEN 
              WRITE(*,*) ' RDLINE: A word exceeds the maximum length.'//
     1                 '  Truncated.'
            END IF
         END IF
C
         IF( NWORDS .GT. MWORDS ) THEN
            WRITE(*,*) ' RDLINE: Too many words found in line.  '//
     1                 'Some ignored. '
            NWORDS = NWORDS - 1
            GO TO 140
         ELSE
