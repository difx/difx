...
#
size=10g
ncontrollers=`lspci -nv | grep "Kernel driver in use: mpt[1-9]sas" | wc -l`
[ $ncontrollers -eq 1 ] && {
    slots=12
}
[ $ncontrollers -eq 2 ] && {
...
[ "$slot" -eq 0 ] && {
    s='' c=0 j=''
    echo "Argument 'slots' is '$slots'"
    [ `expr $slots : '.*1.*'` -gt 0 ] && s="$s 1" && c=$(($c + 1))
    [ `expr $slots : '.*2.*'` -gt 0 ] && s="$s 2" && c=$(($c + 1))
    [ `expr $slots : '.*3.*'` -gt 0 ] && s="$s 3" && c=$(($c + 1))
    [ `expr $slots : '.*4.*'` -gt 0 ] && s="$s 4" && c=$(($c + 1))
    [ `expr $slots : '.*5.*'` -gt 0 ] && s="$s 5" && c=$(($c + 1))
    [ `expr $slots : '.*6.*'` -gt 0 ] && s="$s 6" && c=$(($c + 1))
    [ "$cps" -eq 0 ] && cps=$(($cpus / $c))
    echo The list of slots to condition is $s
    ...
    do
        [ -f /mnt/disks/.meta/$sl/0/eMSN ] &&
           ...
           lab=Module%$sl
        mountpoint -q /mnt/disks/.meta/$sl/0/ && {
           echo "Invoking $0 "$args" slot=$sl label=$lab cpus=$cps"
           $0 "$args" slot=$sl label=$lab cpus=$cps &
           j="$j $!"
        } || {
           echo "Slot $sl disk 0 /mnt/disks/.meta/$sl/0/ is not mounted, skipping this slot!"
        }
    done
...
[ $slot -ge 1 -a $slot -le 6 ] && {
    ...
    do
        [ -d $disk/data ] || {
            echo $label creating data directories $disk/data
            mkdir $disk/data &&
    ...
    do
        [ -d $disk/cull ] || {
            echo $label creating cull directories $disk/cull
            mkdir $disk/cull &&
