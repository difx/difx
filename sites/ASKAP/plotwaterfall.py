...
for stokes in ["I","Q","U","V","XX","YY"]:
    dynspec = np.loadtxt("{0}-imageplane-dynspectrum.stokes{1}.txt".format(src, stokes))
    print dynspec.shape
    
    if nbins == 1:
        f = np.linspace(startfreq, endfreq, endchan)

        ax.plot(f,dynspec[startchan:endchan])
        ...
        dynspec[0] = dynspec[0]/90.
    
        ax.imshow(dynspec[:,startchan:endchan].transpose(), cmap=plt.cm.plasma, interpolation='none', extent=[starttime,endtime,endfreq,startfreq])
