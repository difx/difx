...
polycorefmjdint = int(mjd)
polycorefseconds = int((mjd - int(mjd))*86400 - 1.0)
...
gatestartphase = (86400.0*(gatestartmjd - polycorefmjdfloat) + timediffsec)/inttime
gateendphase = (86400.0*(gateendmjd -  polycorefmjdfloat) + timediffsec)/inttime
...
rfistartphase1 = gatestartphase - 0.02/inttime # RFI gate (early side) starts 20ms before the start of the pulse
rfiendphase1 = gatestartphase - 0.004/inttime # RFI gate (early side) ends 4ms before the start of the pulse
rfistartphase2 = gateendphase + 0.004/inttime # RFI gate (late side) starts 4ms after the end of the pulse
rfiendphase2 = gateendphase + 0.02/inttime # RFI gate (early side) ends 20ms after the end of the pulse
...
binstartmjd = mjd - inttime*pulsewidthms/(2*86400000.0) # pulse width is in ms at this point
gateendmjd = gatestartmjd + pulsewidthms/86400000.0 # pulse width is in ms at this point
binmicrosec = 216
extrawidth = 2 # ms on either side of the snoopy detected pulse
binstartphase = gatestartphase - float(extrawidth)/(1000.0*inttime)
bindeltaphase = binmicrosec/(inttime*1e6)
