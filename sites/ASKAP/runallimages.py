...
for i in range(nbins):
    ...
    if args.cal:
        ...
        print "~/difx/sites/ASKAP/calibrateFRB.py -c {0} -t {1}{2}.FITS {3} {4} -a {5} {6} --uvsrt {7} {8} {9} {10}".format(cdata, tdata[i], bins, cflag, tflag, avgchan, pcen, xpolmodel, noisecen, doimage, refant)
        os.system("rm -rf {0}_calibrated_uv.ms".format(cdatms))
        os.system("~/difx/sites/ASKAP/calibrateFRB.py -c {0} -t {1}{2}.FITS {3} {4} -a {5} {6} --uvsrt {7} {8} {9} {10}".format(cdata, tdata[i], bins, cflag, tflag, avgchan, pcen, xpolmodel, noisecen, doimage, refant))
