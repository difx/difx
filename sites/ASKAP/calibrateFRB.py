...
#                casaout.write('tclean(vis="%s",imagename="%s",imsize=%s,cell=["%.2farcsec", "%.2farcsec"],phasecenter="%s",stokes="%s",specmode="cube",width=%d,interpolation="linear",gridder="widefield",facets=1,psfphasecenter="",chanchunks=1,wprojplanes=-1,vptable="",pblimit=-1,deconvolver="multiscale",scales=[],nterms=1,smallscalebias=0.6,restoration=True,restoringbeam=[''],outlierfile="",weighting="natural",robust=0.0,noise="1.0Jy",npixels=0,uvtaper=[],niter=100,gain=0.1,threshold="0.0mJy",nsigma=0.0,cycleniter=100,cyclefactor=1.5,minpsffraction=0.05,maxpsffraction=0.8,interactive=False,usemask="user",mask=%s,pbmask=0.0,savemodel="modelcolumn")\n' % (targetmsfilename, imagebase, imagesize, pixelsize, pixelsize, options.phasecenter, pol, options.averagechannels, maskstr))
                casaout.write("tclean(vis='{0}', imagename='{1}', imsize={2}, cell=['{3:.2f}arcsec', '{3:.2f}arcsec'], stokes='{4}', specmode='cube', width={5}, phasecenter='{6}', gridder='widefield', wprojplanes=16, pblimit=-1, deconvolver='multiscale', weighting='natural', niter=100, mask={7},restart=True,savemodel='modelcolumn')\n".format(targetmsfilename, imagebase, imagesize, pixelsize, pol, options.averagechannels, options.phasecenter, maskstr))
...
#                casaout.write('clean(vis="%s",imagename=%s,outlierfile="",field="",spw="",selectdata=True,timerange="",uvrange="",mode="channel",gridmode="widefield",wprojplanes=-1,niter=0,gain=0.1,threshold="0.0mJy",psfmode="clark",imagermode="csclean",multiscale=[],interactive=False,mask=%s,nchan=-1,start=1,width=%d,outframe="",veltype="radio",imsize=%s,cell=%s,phasecenter=%s,restfreq="",stokes="%s",weighting="natural",robust=0.0,uvtaper=False,pbcor=False,minpb=0.2,usescratch=False,noise="1.0Jy",npixels=0,npercycle=100,cyclefactor=1.5,cyclespeedup=-1,nterms=1,reffreq="",chaniter=False,flatnoise=True,allowchunk=False)\n' % (targetmsfilename, imagebases, maskstr2, options.averagechannels, imsizestr, cellstr, phasecenterstr, pol))
                casaout.write('tclean(vis='{0}',imagename={1},imsize={2},cell={3},phasecenter="{4}",stokes='{5}',specmode='cube',width={6},gridder='widefield',wprojplanes=16,pblimit=-1,deconvolver='multiscale',weighting='natural',niter=100,cycleniter=100,usemask='user',mask="'{7}'",restart=True,savemodel='modelcolumn')\n'.format(targetmsfilename, imagebases, imsizestr, cellstr, phasecenterstr, pol, options.averagechannels, maskstr2))
            casaout.close()
