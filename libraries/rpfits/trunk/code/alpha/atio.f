C-----------------------------------------------------------------------
C   atio.f: Input/output routines for RPFITS under OSF/1.
C-----------------------------------------------------------------------
C
C   Notes:
...
C-----------------------------------------------------------------------
     ...
     +      form='unformatted', recl=lenrec(lun)/4, iostat=AT_CREATE)
...
C-----------------------------------------------------------------------
     ...
     +      SHARED, form='unformatted', recl=lenrec(lun)/4,
...
C           Skip header file.
     ...
     +            AT_OPEN_READ
         ...
         lenrec(lun) = 2560
         open (lun, file=fname, status='old', access='direct',
     +      form='unformatted', recl=lenrec(lun)/4, iostat=AT_OPEN_READ)
         if (AT_OPEN_READ.ne.0) then
            lenrec(lun) = 512
            open (lun, file=fname, status='old', access='direct',
     +         form='unformatted', recl=lenrec(lun)/4,
     +         iostat=AT_OPEN_READ)
         end if
...
C-----------------------------------------------------------------------
      logical*4 eoff, eotf, errf
      integer*4 fileno, recno, tcsr, tstate, val
...
C           returns 0 if EOF
            ...
            if (AT_READ.eq.-1) then
               type *,'should be an error'
               val = TSTATE(lun, fileno, recno, errf, eoff, eotf, tcsr)
               type *, 'val, fileno, recno, errf, eoff, eotf, tcsr'
               type *,  val, fileno, recno, errf, eoff, eotf, tcsr
            end if

            if (AT_READ.eq.0 .or. AT_READ.eq.-1) then
...
C           WEW - only increment record number if read OK - for online
C           reading where we want to continue reading when new data
C           has arrived

C           Error 36 is attempt to access non-existent record.
C           Error 39 is error during read.  Happens at EOF.
            if (AT_READ.eq.36 .or. AT_READ.eq.39) then
               AT_READ = -1
            else
               irec(lun) = irec(lun) + 1
            end if
...
C        Get the next five 512-byte records.
         if (ISTAPE(lun)) then
            ...
            if (AT_READ.gt.0) AT_READ = TREAD(lun, cbuffer(513:1024))
            if (AT_READ.gt.0) AT_READ = TREAD(lun, cbuffer(1025:1536))
            if (AT_READ.gt.0) AT_READ = TREAD(lun, cbuffer(1537:2048))
            if (AT_READ.gt.0) AT_READ = TREAD(lun, cbuffer(2049:2560))
...
C              clear EOF 'flag' and skip over the EOF+trailer
     ...
     +         iostat=AT_READ) (buffer(j), j=2049,2560)
            ...
            if (AT_READ.eq.36 .or. AT_READ.eq.39) AT_READ = -1
         end if
      end if
...
C-----------------------------------------------------------------------
     ...
     +      'AT_SKIP_EOF:Failed to skip EOF+trailer. Err ', AT_SKIP_EOF
         ...
         do while (AT_SKIP_EOF.ne.-1 .and.
     +             AT_SKIP_EOF.ne.36 .and.
     +             AT_SKIP_EOF.ne.39)
            read (lun, rec=irec(lun), iostat=AT_SKIP_EOF) buffer
            irec(lun) = irec(lun) + 1
         end do

         if (AT_SKIP_EOF.eq.36 .or. AT_SKIP_EOF.eq.39) AT_SKIP_EOF = -1
      end if
      reread = .false.
...
C
C     Tape LUNs are in the range 0 to 3 and bear no relationship to
...
C-----------------------------------------------------------------------
      logical   ISTAPE
      integer   lun, fluns(10:99), tluns(0:3)
...
C-----------------------------------------------------------------------
      integer   fluns(10:99), tluns(0:3)

      common /lunlst/ fluns, tluns
      data  fluns, tluns /90*0, 4*0/
      save /lunlst/
C-----------------------------------------------------------------------
      end
