...
int main(int argc, const char * argv[])
{
  ...
  while(atseconds < config->getExecuteSeconds()  && status > 0)
  {
    ...
    cout << "About to get a visibility now" << endl;
...
void plot_results()
{
  ...
  for(int i=0;i<config->getNumBaselines();i++)
  {
    for(int j=0;j<config->getBNumFreqs(currentconfigindex,i);j++)
    {
	  ...
	  //sprintf(pgplotname, "/home/ssi/amoin/plotmon1.gif/gif");
	  sprintf(pgplotname, "/home/ssi/amoin/mpifxcorr/src/data/monplot-%d-f%d-p%d-b%d.ps/PS",i,j,k,b);
          cpgsubp(1,3);
	//sprintf(pgplotname, "/xs");
          status = cpgopen(pgplotname);
  ...
  for(int i=0;i<config->getNumDataStreams();i++)
  {
    for(int j=0;j<autocorrwidth;j++)
    {
      ...
      {
        //keep plotting...
        cout << "Plotting datastream " << i << ", cross pol " << j << ", band " << k << endl;

	//int numchannels = config->getNumChannels(currentconfigindex);

	if (j==0) {
	  
		  
	 sprintf(pgplotname, "/home/ssi/amoin/mpifxcorr/src/data/monplot-auto%d-b%d.ps/PS",i,k);
	 //sprintf(pgplotname, "/xs"); 
	 status = cpgopen(pgplotname); 
         cpgsubp(1,3);
	  ...
	  sprintf(pgplotname, "/home/ssi/amoin/monplot-cross%d-b%d.ps/PS");
	  //sprintf(pgplotname, "/xs");
	  status = cpgopen(pgplotname); 
	  cpgsubp(1,3);
