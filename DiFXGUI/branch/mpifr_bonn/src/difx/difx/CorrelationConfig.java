...
public class CorrelationConfig
{
    ...
    private boolean commonread, datastreamread, configread, dataoverride,
        updated, usepbs;
    private int baselinetablelength, telescopetablelength,
        datastreamtablelength, freqtablelength;
    private int numbaselines, numdatastreams, numactiveconfigs, numsources,
        defaultconfigindex;
    private int executeseconds, startseconds, startmjd, databufferfactor,
        numdatasegments, numnodes, walltimehours, procspernode;
    private String configfilename, delayfilename, uvwfilename, coreconffilename,
        outputfilename;
    ...
    {
        return DataFormat.formatstrings;
    }
    ...
    {
        return datastreamtable[dsindex].freqtableindices[datastreamtable[
            dsindex].inputbandlocalfreqindices[bandindex]];
    }
    ...
    public boolean productselected(int baselineentry, int freqentry,
                                   int polproductindex)
    {
        ...
        {
            if (bd.datastream1bandindex[j][0] <
                dd.inputbandlocalfreqindices.length &&
                dd.freqtableindices[dd.inputbandlocalfreqindices[bd.
                datastream1bandindex[j][0]]] == freqentry)
            {
                for (int k = 0; k < bd.numpolproducts[j]; k++)
                {
                    if (bd.datastream1bandindex[j][k] <
                        datastreamtable[bd.datastream1index].inputbandpols.
                        length
                        &&
                        bd.datastream2bandindex[j][k] <
                        datastreamtable[bd.datastream2index].inputbandpols.
                        length
                        &&
                        datastreamtable[bd.datastream1index].inputbandpols[bd.
                        datastream1bandindex[j][k]]
                        == polpairs[polproductindex].charAt(0)
                        &&
                        datastreamtable[bd.datastream2index].inputbandpols[bd.
                        datastream2bandindex[j][k]]
    ...
    public boolean productselectionpossible(int ds1index, int ds2index,
                                            int freqentry, int polproductindex)
    {
    ...
    public boolean productselectionpossible(int baselineentry, int freqentry,
                                            int polproductindex)
    {
        BaselineData bd = baselinetable[baselineentry];

        return productselectionpossible(bd.datastream1index,
                                        bd.datastream2index, freqentry,
                                        polproductindex);
    }
    ...
    void setDatastreamFreqClockOffset(int datastreamindex, int freq,
                                      double offset)
    {
    ...
    {
        System.out.println("About to set filenames for " + datastreamindex +
                           ", length " + filenames.length);
        datastreamtable[datastreamindex].numdatafiles = (filenames == null) ? 0 :
            filenames.length;
    ...
    {
        updated = false;
        int smaller = (newlength < freqtablelength) ? newlength :
            freqtablelength;
    ...
    public void setFreqIndices(int datastreamindex, int[] freqindices)
    {
        ...
        datastreamtable[datastreamindex].inputbandlocalfreqindices =
            localfreqindices;
        ...
        {
            datastreamtable[datastreamindex].freqtableindices[i] =
                uniqueindices[i];
            datastreamtable[datastreamindex].freqpols[i] = numpols[i];
            if (i < oldoffsets.length)
                datastreamtable[datastreamindex].freqclockoffsets[i] =
                    oldoffsets[i];
            else
    ...
    public void reset()
    {
        baselinetablelength = telescopetablelength = datastreamtablelength =
            freqtablelength = 0;
        numbaselines = numdatastreams = defaultconfigindex = numconfigsloaded =
            numsources = 0;
        ...
        databufferfactor = 32;
        numdatasegments = 8;
        delayfilename = uvwfilename = coreconffilename = outputfilename =
            outputformatstring = "";
    ...
    public void addNewDatastream(int telescopeindex)
    {
        ...
        datastreamtable[datastreamtablelength -
            1] = new DatastreamData(telescopetable, freqtable);
        datastreamtable[datastreamtablelength -
            1].telescopeindex = telescopeindex;
        for (int i = 0; i < baselinetablelength; i++)
    ...
    {
        datastreamtable[targetindex].format = datastreamtable[sourceindex].
            format;
        datastreamtable[targetindex].numinputbands = datastreamtable[
            sourceindex].numinputbands;
        datastreamtable[targetindex].numfreqs = datastreamtable[sourceindex].
            numfreqs;
        datastreamtable[targetindex].readfromfile = datastreamtable[sourceindex].
            readfromfile;
        datastreamtable[targetindex].portnumber = datastreamtable[sourceindex].
            portnumber;
        datastreamtable[targetindex].tcpwindowsizekb = datastreamtable[
            sourceindex].tcpwindowsizekb;
        datastreamtable[targetindex].freqpols = new int[datastreamtable[
            sourceindex].freqpols.length];
        for (int i = 0; i < datastreamtable[sourceindex].freqpols.length; i++)
            datastreamtable[targetindex].freqpols[i] = datastreamtable[
                sourceindex].freqpols[i];
        datastreamtable[targetindex].freqtableindices = new int[datastreamtable[
            sourceindex].freqtableindices.length];
        for (int i = 0;
             i < datastreamtable[sourceindex].freqtableindices.length; i++)
            datastreamtable[targetindex].freqtableindices[i] = datastreamtable[
                sourceindex].freqtableindices[i];
        datastreamtable[targetindex].inputbandlocalfreqindices = new int[
            datastreamtable[sourceindex].inputbandlocalfreqindices.length];
        for (int i = 0;
             i < datastreamtable[sourceindex].inputbandlocalfreqindices.length;
             i++)
            datastreamtable[targetindex].inputbandlocalfreqindices[i] =
                datastreamtable[sourceindex].inputbandlocalfreqindices[i];
        datastreamtable[targetindex].inputbandpols = new char[datastreamtable[
            sourceindex].inputbandpols.length];
        for (int i = 0; i < datastreamtable[sourceindex].inputbandpols.length;
             i++)
            datastreamtable[targetindex].inputbandpols[i] = datastreamtable[
                sourceindex].inputbandpols[i];
    }

    public void setBaseline(int tableindex, int datastream1index,
                            int datastream2index, boolean[][] products)
    {
        ...
        baselinetable[tableindex] = new BaselineData(telescopetable, freqtable,
            datastreamtable);
        ...
        for (int i = 0; i < products.length; i++)
        {
            if (nproducts[i] > 0)
            {
                ...
                baselinetable[tableindex].datastream1bandindex[count] = new int[
                    nproducts[i]];
                baselinetable[tableindex].datastream2bandindex[count] = new int[
                    nproducts[i]];
                for (int j = 0; j < 4; j++)
                {
                    if (products[i][j])
                    {
                        ...
                        {
                            if (ds1.freqtableindices[ds1.
                                inputbandlocalfreqindices[k]] == i
                                && polpairs[j].charAt(0) == ds1.inputbandpols[k])
                                ds1bandindex = k;
                        }
                        for (int k = 0; k < ds2.numinputbands; k++)
                        {
                            if (ds2.freqtableindices[ds2.
                                inputbandlocalfreqindices[k]] == i
                        ...
                        if (ds1bandindex < 0 || ds2bandindex < 0)
                            System.err.println("Something has gone badly awry - I can't find the datastream bands for baseline " +
                                               ...
                                               "'th frequency, " + j +
                                               "'th band!!!");
                        baselinetable[tableindex].datastream1bandindex[count][
                            pcount] = ds1bandindex;
                        baselinetable[tableindex].datastream2bandindex[count][
                            pcount] = ds2bandindex;
                        pcount++;
                    }
                }
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            for (int j = 0; j < datastreamtable[i].numfreqs; j++)
            {
                if (datastreamtable[i].freqtableindices[j] == toremove)
                {
                    ...
                    if (toreturn == null)
                        toreturn = "";
                    toreturn += telescopetable[datastreamtable[i].
                        telescopeindex].name + ": index " + j;
                }
            }
        }
    ...
    public void removeDatastream(int datastreamindex)
    {
        ...
        for (int i = 0; i < datastreamtablelength - 1; i++)
            datastreamtable[i] = (i < datastreamindex) ? olddstable[i] :
                olddstable[i + 1];
    ...
    public void setCommonSettings(String delayfilename, String uvwfilename,
                                  int startmjd, int startseconds,
                                  int executeseconds,
                                  int numdatastreams, int numbaselines,
                                  int numdatasegments,
    ...
    public void setNodeSettings(boolean usepbs, String clustername,
    ...
    public void setTelescopeTableData(String[][] data)
    {
        ...
        if (data != null)
        {
            ...
            {
                System.out.println("Saving data " + data[i][0] + " " +
                                   data[i][1] + " " + data[i][2]);
    ...
    public void removeTelescope(String telescopename)
    {
        ...
        if (telescopeid >= 0)
        {
            ...
            for (int i = 0 - removecount; i < baselinetablelength - removecount;
                 i++)
            {
    ...
    public void trimDataFilesBySource()
    {
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            datastreamtable[i].datafilenames = ft.trimDataFiles(datastreamtable[
                i].datafilenames);
            datastreamtable[i].numdatafiles = datastreamtable[i].datafilenames.
                length;
        }
    }
    ...
    {
        FileTrimmer ft = new FileTrimmer(startmjd, startseconds, starts, stops,
                                         numscans);
        for (int i = 0; i < datastreamtablelength; i++)
        {
            datastreamtable[i].datafilenames = ft.trimDataFiles(datastreamtable[
    ...
    public void loadCorrConfigFile(String filename)
    {
        try
        {
            ...
            {
                System.out.println(
                    "Error - no config section in input file - aborting!!!");
                return;
            }
            input.close();
    ...
    public void saveCorrConfigFile(String filename)
    {
        ...
        {
            JOptionPane.showMessageDialog(mainwindow,
                "Inconsistent data found in configuration: " + e.getMessage(),
                                          "Configuration consistency error!!",
                                          JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void writeConfigTable(PrintWriter output)
    {
        ...
        for (int i = 0; i < configs.length; i++)
        {
            if (configs[i].active)
            {
                writeoutputline(output, configs[i].sourcename.trim(),
                                "CONFIG SOURCE");
                writeoutputline(output, configs[i].inttime, "INT TIME (SEC)");
                writeoutputline(output, configs[i].numchannels, "NUM CHANNELS");
                writeoutputline(output, configs[i].blockspersend,
                                "BLOCKS PER SEND");
                writeoutputline(output, configs[i].guardblocks, "GUARD BLOCKS");
                writeoutputline(output, configs[i].postffringerot,
                                "POST-F FRINGE ROT");
                writeoutputline(output, configs[i].quadraticdelayinterp,
                                "QUAD DELAY INTERP");
                writeoutputline(output, configs[i].writeautocorrs,
                                "WRITE AUTOCORRS");
                writeoutputline(output, configs[i].pulsarbin, "PULSAR BINNING");
                if (configs[i].pulsarbin)
                    writeoutputline(output, configs[i].pulsarfilename,
                                    "PULSAR CONFIG FILE");
                for (int j = 0; j < numdatastreams; j++)
                    writeoutputline(output, configs[i].datastreamindices[j],
                                    "DATASTREAM " + j + " INDEX");
                for (int j = 0; j < numbaselines; j++)
                    writeoutputline(output, configs[i].baselineindices[j],
                                    "BASELINE " + j + " INDEX");
            }
        }
        output.println();
    ...
    private void writeFreqTable(PrintWriter output)
    {
        ...
        for (int i = 0; i < freqtablelength; i++)
        {
            writeoutputline(output, freqtable[i].bandedgefreq,
                            "FREQ (MHZ) " + i);
            writeoutputline(output, freqtable[i].bandwidth, "BW (MHZ) " + i);
            writeoutputline(output, ( (freqtable[i].lowersideband) ? "L" : "U"),
                            "SIDEBAND " + i);
        }
        output.println();
    }
    ...
    private void writeTelescopeTable(PrintWriter output)
    {
        ...
        for (int i = 0; i < telescopetablelength; i++)
        {
            writeoutputline(output, telescopetable[i].name,
                            "TELESCOPE NAME " + i);
            writeoutputline(output, telescopetable[i].clockdelay,
                            "CLOCK DELAY (us) " + i);
            writeoutputline(output, telescopetable[i].clockrate,
                            "CLOCK RATE(us/s) " + i);
        }
        output.println();
    }
    ...
    private void writeDatastreamTable(PrintWriter output)
    {
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            writeoutputline(output, datastreamtable[i].telescopeindex,
                            "TELESCOPE INDEX");
            ...
            writeoutputline(output, datastreamtable[i].numbits,
                            "QUANTISATION BITS");
            writeoutputline(output, datastreamtable[i].filterbank,
                            "FILTERBANK USED");
            writeoutputline(output, datastreamtable[i].readfromfile,
                            "READ FROM FILE");
            writeoutputline(output, datastreamtable[i].numfreqs, "NUM FREQS");
            for (int j = 0; j < datastreamtable[i].numfreqs; j++)
            {
                writeoutputline(output, datastreamtable[i].freqtableindices[j],
                                "FREQ TABLE INDEX " + j);
                writeoutputline(output, datastreamtable[i].freqclockoffsets[j],
                                "CLK OFFSET " + j + " (us)");
                writeoutputline(output, datastreamtable[i].freqpols[j],
                                "NUM POLS " + j);
            }
            for (int j = 0; j < datastreamtable[i].numinputbands; j++)
            {
                writeoutputline(output, datastreamtable[i].inputbandpols[j],
                                "INPUT BAND " + j + " POL");
                writeoutputline(output,
                                datastreamtable[i].inputbandlocalfreqindices[j],
                                "INPUT BAND " + j + " INDEX");
            }
        }
        output.println();
    ...
    private void writeBaselineTable(PrintWriter output)
    {
        ...
        for (int i = 0; i < baselinetablelength; i++)
        {
            writeoutputline(output, baselinetable[i].datastream1index,
                            "D/STREAM A INDEX " + i);
            writeoutputline(output, baselinetable[i].datastream2index,
                            "D/STREAM B INDEX " + i);
            writeoutputline(output, baselinetable[i].numfreqs, "NUM FREQS " + i);
            for (int j = 0; j < baselinetable[i].numfreqs; j++)
            {
                writeoutputline(output, baselinetable[i].numpolproducts[j],
                                "POL PRODUCTS " + i + "/" + j);
                for (int k = 0; k < baselinetable[i].numpolproducts[j]; k++)
                {
                    writeoutputline(output,
                                    baselinetable[i].datastream1bandindex[j][k],
                                    "D/STREAM A BAND " + k);
                    writeoutputline(output,
                                    baselinetable[i].datastream2bandindex[j][k],
                                    "D/STREAM B BAND " + k);
                }
            }
        }
    ...
    private void writeSourceDataTable(PrintWriter output)
    {
        ...
        if (dodatatable)
        {
            ...
            for (int i = 0; i < datastreamtablelength; i++)
            {
                writeoutputline(output, datastreamtable[i].numdatafiles,
                                "D/STREAM " + i + " FILES");
                for (int j = 0; j < datastreamtable[i].numdatafiles; j++)
                    writeoutputline(output, datastreamtable[i].datafilenames[j],
                                    "FILE " + i + "/" + j);
            }
            output.println();
        }
        ...
        if (donetworktable)
        {
            ...
            for (int i = 0; i < datastreamtablelength; i++)
            {
                writeoutputline(output, datastreamtable[i].portnumber,
                                "PORT NUM");
                writeoutputline(output, datastreamtable[i].tcpwindowsizekb,
                                "TCP WINDOW SIZE");
            }
            output.println();
        }
    ...
    private void processCommon(BufferedReader input) throws IOException,
        ConsistencyException
    {
        ...
        dataoverride = (line.equalsIgnoreCase("TRUE") ||
                        line.equalsIgnoreCase("T")) ? true : false;
    ...
    private void processConfig(BufferedReader input) throws IOException
    {
        ...
        for (int i = 0; i < numactiveconfigs; i++)
        {
            ...
            {
                System.out.println("Comparing " + configs[j].sourcename.trim() +
                                   " with " + line.trim());
            ...
            if (configindex >= 0)
            {
                ...
                configs[configindex].ordereddatastreamindices = new int[
                    numdatastreams];
                ...
                configs[configindex].postffringerot = (line.equalsIgnoreCase(
                    "TRUE") || line.equalsIgnoreCase("T")) ? true : false;
                line = getinputline(input, "QUAD DELAY INTERP");
                configs[configindex].quadraticdelayinterp = (line.
                    equalsIgnoreCase("TRUE") || line.equalsIgnoreCase("T")) ? true : false;
                if (configs[configindex].postffringerot &&
                    configs[configindex].quadraticdelayinterp)
                {
                ...
                configs[configindex].writeautocorrs = (line.equalsIgnoreCase(
                    "TRUE") || line.equalsIgnoreCase("T")) ? true : false;
                line = getinputline(input, "PULSAR BINNING");
                configs[configindex].pulsarbin = (line.equalsIgnoreCase("TRUE") ||
                                                  line.equalsIgnoreCase("T")) ? true : false;
                if (configs[configindex].pulsarbin)
                {
                    configs[configindex].pulsarfilename = getinputline(input,
                        "PULSAR CONFIG FILE");
                }
                for (int j = 0; j < numdatastreams; j++)
                {
                    line = getinputline(input, "DATASTREAM");
                    configs[configindex].datastreamindices[j] = Integer.valueOf(
                        line);
                }
                for (int j = 0; j < numbaselines; j++)
                {
                    line = getinputline(input, "BASELINE");
                    configs[configindex].baselineindices[j] = Integer.valueOf(
                        line);
                }
            }
        }
    ...
    private void processFreqTable(BufferedReader input) throws IOException
    {
        ...
        for (int i = 0; i < freqtablelength; i++)
        {
            ...
            freqtable[i].lowersideband = (line.equalsIgnoreCase("L") ||
                                          line.equalsIgnoreCase("LOWER")) ? true : false;
        }
    }
    ...
    private void processDatastreamTable(BufferedReader input) throws
        IOException
    {
        String line;
        ...
        {
            System.err.println(
                "Error - not enough datastreams are supplied in the datastream table (" +
                datastreamtablelength +
                ") compared to the number of datastreams (" + numdatastreams +
                "!!!");
            return;
        }
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            ...
            datastreamtable[i].filterbank = (line.equalsIgnoreCase("TRUE") ||
                                             line.equalsIgnoreCase("T")) ? true : false;
            ...
            datastreamtable[i].readfromfile = (line.equalsIgnoreCase("TRUE") ||
                                               line.equalsIgnoreCase("T")) ? true : false;
            ...
            datastreamtable[i].freqtableindices = new int[datastreamtable[i].
                numfreqs];
            datastreamtable[i].freqclockoffsets = new double[datastreamtable[i].
                numfreqs];
            ...
            for (int j = 0; j < datastreamtable[i].numfreqs; j++)
            {
                ...
                datastreamtable[i].numinputbands += datastreamtable[i].freqpols[
                    j];
            }
            datastreamtable[i].bytespersamplenum = (datastreamtable[i].
                numinputbands * datastreamtable[i].numbits) / 8;
            if (datastreamtable[i].bytespersamplenum == 0)
            {
                datastreamtable[i].bytespersamplenum = 1;
                datastreamtable[i].bytespersampledenom = 8 /
                    (datastreamtable[i].numinputbands *
                     datastreamtable[i].numbits);
            }
            else
                datastreamtable[i].bytespersampledenom = 1;
            if (!datastreamtable[i].filterbank)
                datastreamtable[i].numoutputbands = datastreamtable[i].
                    numinputbands;
            datastreamtable[i].inputbandpols = new char[datastreamtable[i].
                numinputbands];
            datastreamtable[i].inputbandlocalfreqindices = new int[
                datastreamtable[i].numinputbands];
            for (int j = 0; j < datastreamtable[i].numinputbands; j++)
            {
                ...
                line = getinputline(input, "INPUT BAND");
                datastreamtable[i].inputbandlocalfreqindices[j] = Integer.
                    valueOf(line);
                if (datastreamtable[i].inputbandlocalfreqindices[j] >=
                    datastreamtable[i].numfreqs)
                    System.err.println(
                        "Error - attempting to refer to freq outside local table!!!");
            }
        }
        if (datastreamtable[0].inputbandpols[0] == 'X' ||
            datastreamtable[0].inputbandpols[0] == 'Y' ||
            datastreamtable[0].inputbandpols[0] == 'x' ||
            datastreamtable[0].inputbandpols[0] == 'y')
            polpairs = new String[]
                {
                "XX", "YY", "XY", "YX"};
    ...
    private void processBaselineTable(BufferedReader input) throws IOException
    {
        ...
        {
            System.err.println(
                "Error - not enough baselines are supplied in the baseline table (" +
                baselinetablelength + ") compared to the number of baselines (" +
                numbaselines + ")!!!");
            return;
        }
        ...
        for (int i = 0; i < baselinetablelength; i++)
        {
            ...
            baselinetable[i] = new BaselineData(telescopetable, freqtable,
                                                datastreamtable);
            ...
            baselinetable[i].datastream1bandindex = new int[baselinetable[i].
                numfreqs][];
            baselinetable[i].datastream2bandindex = new int[baselinetable[i].
                numfreqs][];
            for (int j = 0; j < baselinetable[i].numfreqs; j++)
            {
                line = getinputline(input, "POL PRODUCTS");
                baselinetable[i].numpolproducts[j] = Integer.valueOf(line.trim());
                baselinetable[i].datastream1bandindex[j] = new int[
                    baselinetable[i].numpolproducts[j]];
                baselinetable[i].datastream2bandindex[j] = new int[
                    baselinetable[i].numpolproducts[j]];
                for (int k = 0; k < baselinetable[i].numpolproducts[j]; k++)
                {
                    baselinetable[i].totalbands++;
                    line = getinputline(input, "D/STREAM A BAND");
                    baselinetable[i].datastream1bandindex[j][k] = Integer.
                        valueOf(line);
                    line = getinputline(input, "D/STREAM B BAND");
                    baselinetable[i].datastream2bandindex[j][k] = Integer.
                        valueOf(line);
                }
            }
            if (datastreamtable[baselinetable[i].datastream1index].
                telescopeindex >
                datastreamtable[baselinetable[i].datastream2index].
                telescopeindex)
            {
                System.err.println("Error - first datastream for baseline " + i +
                    " has a higher number than second datastream - reversing!!!");
                tempint = baselinetable[i].datastream1index;
                baselinetable[i].datastream1index = baselinetable[i].
                    datastream2index;
                baselinetable[i].datastream2index = tempint;
                tempintptr = baselinetable[i].datastream1bandindex;
                baselinetable[i].datastream1bandindex = baselinetable[i].
                    datastream2bandindex;
    ...
    private void processDataTable(BufferedReader input) throws IOException
    {
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            ...
            datastreamtable[i].datafilenames = new String[datastreamtable[i].
                numdatafiles];
            for (int j = 0; j < datastreamtable[i].numdatafiles; j++)
                datastreamtable[i].datafilenames[j] = getinputline(input,
                    "FILE");
        }
    }
    ...
    public void consistencyCheck(boolean strict) throws ConsistencyException
    {
        ...
        if (strict)
        {
            ...
            catch (FileNotFoundException e)
            {
                throw new ConsistencyException(
                    "Configuration file could not be opened: " + e.getMessage());
            }
            catch (IOException e)
            {
                throw new ConsistencyException(
                    "Configuration file was not read successfully: " +
                    e.getMessage());
            }
        }
        if (! (outputformatstring.equals("RPFITS") ||
               outputformatstring.equals("ASCII") ||
               outputformatstring.equals("SWIN")))
            throw new ConsistencyException(
                "Output format must be RPFITS or ASCII or SWIN - found " +
                outputformatstring);
        ...
        for (int i = 0; i < datastreamtablelength; i++)
        {
            ...
            if (datastreamtable[i].telescopeindex < 0 ||
                datastreamtable[i].telescopeindex >= telescopetablelength)
            {
                throw new ConsistencyException("Datastream table entry " + i +
                    " has a telescope index that refers outside the telescope table range ("
                                               +
            ...
            for (int j = 0; j < datastreamtable[i].numinputbands; j++)
            {
                if (datastreamtable[i].inputbandlocalfreqindices[j] < 0 ||
                    datastreamtable[i].inputbandlocalfreqindices[j] >=
                    datastreamtable[i].numfreqs)
                {
                    throw new ConsistencyException("Datastream table entry " +
                        i + " has an input band local frequency index (band " +
                        j
                        +
                        ") that refers outside the local frequency table range (" +
                                                 datastreamtable[i].
                                                 inputbandlocalfreqindices[j] +
                                                 ")");
                }
            }

            //check the frequency table indices are ok and all the bandwidths all match
            bandwidth = freqtable[datastreamtable[i].freqtableindices[0]].
                bandwidth;
            for (int j = 0; j < datastreamtable[i].numfreqs; j++)
            {
                if (datastreamtable[i].freqtableindices[j] < 0 ||
                    datastreamtable[i].freqtableindices[j] >= freqtablelength)
                {
                    throw new ConsistencyException("Datastream table entry " +
                        i + " has a frequency index (freq " + j +
                        ") that refers outside the frequency table range ("
                        + datastreamtable[i].freqtableindices[j] + ")");
                }
                if (bandwidth !=
                    freqtable[datastreamtable[i].freqtableindices[j]].bandwidth)
                {
                    throw new ConsistencyException(
                        "All bandwidths for a given datastream must be equal");
                }
            }
        }
        ...
        for (int i = 0; i < numdatastreams; i++)
        {
            ...
            for (int j = 0; j < configs.length; j++)
            {
                ...
                {
                    if (tindex == -1)
                        tindex = datastreamtable[configs[j].datastreamindices[i]].
                            telescopeindex;
                    else if (tindex !=
                             datastreamtable[configs[j].datastreamindices[i]].
                             telescopeindex)
                    {
                        throw new ConsistencyException(
                            "All configs must have the same telescopes!  Config " +
                            j + " datastream " + i +
                            " refers to different telescopes");
                    }
                }
            }
        }
...
//			work out the ordereddatastreamindices
                ...
                {
                    throw new ConsistencyException(
                        "Not all datastreams accounted for in the datastream table for config " +
                        i);
                }
                ...
                {
                    sampletimens = 1000.0 /
                        freqtable[datastreamtable[configs[i].datastreamindices[
                        j]].freqtableindices[0]].bandwidth;
                    ffttime = sampletimens * configs[i].numchannels * 2;
                    nsincrement = ffttime * configs[i].blockspersend *
                        (databufferfactor / numdatasegments);
                    if (ffttime - (int) (ffttime + 0.5) > 0.00000001 ||
                        ffttime - (int) (ffttime + 0.5) < -0.000000001)
                    {
                        throw new ConsistencyException(
                            "FFT chunk time for config " + i + ", datastream " +
                            j + " is not a whole number of nanoseconds ("
                            + ffttime + ")");
                    }
                    if (nsincrement > Integer.MAX_VALUE)
                    {
                        throw new ConsistencyException(
                            "Increment per read in nanoseconds is " +
                            nsincrement + " - too large to fit in an int");
                    }
                }
                ...
                for (int j = 0; j < numbaselines; j++)
                {
                    if (configs[i].baselineindices[j] < 0 ||
                        configs[i].baselineindices[j] >= baselinetablelength) //bad index
                    {
                        throw new ConsistencyException("Config " + i +
                            " baseline index " + j + " refers to baseline " +
                            configs[i].baselineindices[j]
                            +
        ...
        for (int i = 0; i < baselinetablelength; i++)
        {
            ...
            if (baselinetable[i].datastream1index < 0 ||
                baselinetable[i].datastream2index < 0 ||
                baselinetable[i].datastream1index >=
                datastreamtablelength ||
                baselinetable[i].datastream2index >= datastreamtablelength)
            {
            ...
            for (int j = 0; j < baselinetable[i].numfreqs; j++)
            {
                for (int k = 0; k < baselinetable[i].numpolproducts[j]; k++)
                {
                    if (baselinetable[i].datastream1bandindex[j][k] < 0 ||
                        baselinetable[i].datastream1bandindex[j][k] >=
                        datastreamtable[baselinetable[i].datastream1index].
                        numinputbands)
                    {
                        throw new ConsistencyException("Baseline table entry " +
                            i + ", frequency " + j + ", polarisation product " +
                            k
                            +
                            " for datastream 1 refers to a band outside datastream 1's range (" +
                            baselinetable[i].datastream1bandindex[j][k] + ")");
                    }
                    if (baselinetable[i].datastream2bandindex[j][k] < 0 ||
                        baselinetable[i].datastream2bandindex[j][k] >=
                        datastreamtable[baselinetable[i].datastream2index].
                        numinputbands)
                    {
                        throw new ConsistencyException("Baseline table entry " +
                            i + ", frequency " + j + ", polarisation product " +
                            k +
                            " for datastream 2 refers to a band outside datastream 2's range (" +
                            baselinetable[i].datastream2bandindex[j][k] + ")");
                    }
                }
            }
        ...
        if (databufferfactor % numdatasegments != 0)
        {
            throw new ConsistencyException("There must be an integer number of sends per datasegment.  Presently databufferfactor is "
                                           + databufferfactor +
                                           ", and numdatasegments is " +
                                           numdatasegments);
        }
    }

    private void checkDelayFile(boolean loadsources) throws IOException,
        ConsistencyException
    {
        Vector<String> scannames = new Vector<String> ();
        String source;
        int year, month, day, hour, minute, second, incrementsecs,
            numdelaytelescopes,
            numscans, scanpoints, totalpoints, delaymjd, delaystartseconds,
            delaylengthseconds;
        boolean found;
        ...
        numdelaytelescopes = Integer.valueOf(getinputline(input,
            "NUM TELESCOPES"));
            ...
            for (int i = 0; i < configs.length; i++)
            {
                if (configs[i].active)
                {
                    for (int j = 0; j < numdatastreams; j++)
                    {
                        ...
                        {
                            if (telescopenames[k].trim().equals(telescopetable[
                                datastreamtable[configs[i].datastreamindices[j]].
                                telescopeindex].name.trim()))
                                found = true;
                        }
                        if (!found)
                            throw new ConsistencyException("Telescope " +
                                telescopetable[datastreamtable[configs[i].
                                datastreamindices[j]].telescopeindex].name +
                                " could not be found in the delay file!!!");
                    }
                }
            }
        }
        ...
        if ( (startmjd - delaymjd) * 86400 + startseconds - delaystartseconds <
            0 ||
            (startmjd - delaymjd) * 86400 + startseconds + executeseconds -
            (delaystartseconds + delaylengthseconds) > 0)
            throw new ConsistencyException("Experiment runs from MJD " +
                                           startmjd + " + " + startseconds +
                                           " seconds to MJD " +
                                           (startmjd +
                                            ( (startseconds + executeseconds) /
                                             86400)) + " + " +
                                           ( (startseconds + executeseconds) %
                                            86400) +
                " seconds, while delay file runs from MJD " +
                                           delaymjd + " + " + delaystartseconds +
                                           " to MJD " +
                                           (delaymjd +
                                            ( (delaystartseconds + delaylengthseconds) /
                                             86400)) +
                                           " + " +
                                           ( (delaystartseconds + delaylengthseconds) %
                                            86400) + " seconds");
    ...
    private void checkUVWFile() throws IOException, ConsistencyException
    {
        int year, month, day, hour, minute, second, incrementsecs,
            numuvwtelescopes,
            numscans, scanpoints, totalpoints, uvwmjd, uvwstartseconds,
            uvwlengthseconds;
        boolean found;
        String[] telescopenames;
                ...
                for (int j = 0; j < numdatastreams; j++)
                {
                    ...
                    if (!found)
                        throw new ConsistencyException("Telescope " +
                            telescopetable[datastreamtable[configs[i].
                            datastreamindices[j]].telescopeindex].name +
                            " could not be found in the uvw file!!!");
                }
            }
        }
        ...
        if ( (startmjd - uvwmjd) * 86400 + startseconds - uvwstartseconds < 0 ||
            (startmjd - uvwmjd) * 86400 + startseconds + executeseconds -
            (uvwstartseconds + uvwlengthseconds) > 0)
            throw new ConsistencyException("Experiment runs from MJD " +
                                           startmjd + " + " + startseconds +
                                           " seconds to MJD " +
                                           (startmjd +
                                           ...
                                           ( (startseconds + executeseconds) %
                                            86400) +
                " seconds, while delay file runs from MJD " +
                                           uvwmjd + " + " + uvwstartseconds +
                                           " to MJD " +
                                           (uvwmjd +
                                            ( (uvwstartseconds + uvwlengthseconds) /
                                             86400)) +
                                           " + " +
                                           ( (uvwstartseconds + uvwlengthseconds) %
                                            86400) + " seconds");
    ...
    private void checkThreadFile() throws IOException, ConsistencyException
    {
        ...
        if (numnodes - (numdatastreams + 1) > maxcores)
            throw new ConsistencyException("Thread file " + coreconffilename +
                                           " specifies a max of " +
                                           maxcores + " cores, while " +
                                           (numnodes - numdatastreams - 1) +
                                           " are to be used");
    }
    ...
    public String getinputline(BufferedReader input, String startofheader) throws
        IOException
    {
        char[] cbuf = new char[DiFXManager.KEYWORD_LENGTH];
        input.read(cbuf, 0, DiFXManager.KEYWORD_LENGTH);
        if (! (new String(cbuf).substring(0,
                                          startofheader.length()).equals(
            startofheader))) //not what we expected
            System.err.println(
                "Error - we thought we were reading something starting with '" +
                startofheader + "', when we actually got '" + new String(cbuf));
        return input.readLine().trim();
    }

    private void writeoutputline(PrintWriter output, String value,
                                 String keyword)
    {
        output.println(keyword + ":" + blanks.substring(keyword.length() + 1) +
                       value);
    }
    ...
    private void writeoutputline(PrintWriter output, boolean value,
                                 String keyword)
    {
        writeoutputline(output, Boolean.toString(value).toUpperCase(), keyword);
    }

    private void writeoutputline(PrintWriter output, double value,
                                 String keyword)
    {
    ...
    private class FreqData
    {
        ...
        {
            return bandedgefreq + " " + bandwidth + " " +
                Boolean.toString(lowersideband);
        }
    }
    ...
    private class BaselineData
    {
        ...
        public BaselineData(TelescopeData[] tt, FreqData[] ft,
                            DatastreamData[] dt)
        {
            telescopetable = tt;
            freqtable = ft;
        ...
        public String toString()
        {
            ...
            String toreturn = "BASELINE:           " +
                telescopetable[datastreamtable[datastream1index].telescopeindex].
                name +
                "-" +
                telescopetable[datastreamtable[datastream2index].telescopeindex].
                name + "\n" +
                "NUMBER OF FREQS:    " + numfreqs + "\n";
            for (int i = 0; i < numfreqs; i++)
            {
                toreturn += freqtable[d1.freqtableindices[d1.
                    inputbandlocalfreqindices[datastream1bandindex[i][0]]]].
                    bandedgefreq + ":             ";
                for (int j = 0; j < numpolproducts[i]; j++)
                {
                    toreturn +=
                        (d1.inputbandpols[datastream1bandindex[i][j]] + "") +
                        (d2.inputbandpols[datastream2bandindex[i][j]] + "") +
                        " ";
                }
                toreturn += "\n";
            }
        ...
        {
            return telescopetable[datastreamtable[datastream1index].
                telescopeindex].name +
                "-" +
                telescopetable[datastreamtable[datastream2index].telescopeindex].
                name;
    ...
    private class ConfigData
    {
        ...
        public String toString()
        {
            ...
            toreturn += "POST-F FRINGE ROT:  " + Boolean.valueOf(postffringerot) +
                "\n";
            toreturn += "QUAD DELAY INTERP:  " +
                Boolean.valueOf(quadraticdelayinterp) + "\n";
            ...
            if (pulsarbin)
            {
                toreturn += "SCRUNCH OUTPUT:     " +
                    Boolean.valueOf(scrunchoutput) + "\n";
                toreturn += "PULSAR FILENAME:    " + pulsarfilename + "\n";
                for (int i = 0; i < numpolycos; i++)
                    toreturn += "POLYCO " + i + " FILENAME:    " +
                        polycofilenames[i] + "\n";
            }
            for (int i = 0; i < numdatastreams; i++)
            {
                toreturn += "DATASTREAM " + i + " INDEX: " +
                    datastreamindices[i] + "\n";
            }
    ...
    private class DatastreamData
    {
        ...
        public DatastreamData(TelescopeData[] td, FreqData[] ft)
        {
            freqtable = ft;
            telescopetable = td;
            numfreqs = numinputbands = numdatafiles = portnumber =
                tcpwindowsizekb = fanout = numbits = 0;
            format = 0;
            readfromfile = true;
            filterbank = false;
        }
        ...
        public String toString()
        {
            String toreturn = "TELESCOPE:          " +
                telescopetable[telescopeindex].name + "\n" +
                "TSYS (JY):          " + tsys + "\n" +
                "FORMAT:             " + DataFormat.formatstrings[format] +
                "\n" +
                "NUMBER OF BITS:     " + numbits + "\n" +
                ( (format == DataFormat.MKV) ?
                 ("FANOUT:             " + fanout + "\n") : "") +
            ...
            {
                toreturn += "BAND " + i + ":             " +
                    freqtable[inputbandlocalfreqindices[i]].bandedgefreq + " " +
                    freqtable[inputbandlocalfreqindices[i]].bandwidth + " " +
                    inputbandpols[i] + "\n";
            }
            if (readfromfile)
        ...
        public String getBandSummary()
        {
            ...
            for (int i = 0; i < numinputbands; i++)
            {
                toreturn += freqtable[inputbandlocalfreqindices[i]].
                    bandedgefreq + ":" + inputbandpols[i] + ",";
            }
    ...
    private class FileTrimmer
    {
        ...
        public FileTrimmer(int startmjd, int startseconds, int[] modestarts,
                           int[] modestops, int nmodescans)
        {
        ...
        {
            int year, month, day, hour, minute, second, numuvwtelescopes,
                numscans, scanpoints;
            ...
            for (int i = 0; i < configs.length; i++)
            {
                if (configs[i].sourcename.trim().equals("DEFAULT") &&
                    configs[i].active)
                    passall = true;
            }
            ...
            try
            {
                ...
                hour = Integer.valueOf(getinputline(input, "START HOUR"));
                minute = Integer.valueOf(getinputline(input, "START MINUTE"));
                second = Double.valueOf(getinputline(input, "START SECOND")).
                    intValue();
                startmjd = ymd2mjd(year, month, day);
                startdoy = startmjd - ymd2mjd(year, 1, 1) + 1;
                startseconds = hour * 3600 + minute * 60 + second;
                incrementsecs = Integer.valueOf(getinputline(input,
                    "INCREMENT (SECS)"));
                numuvwtelescopes = Integer.valueOf(getinputline(input,
                    "NUM TELESCOPES"));
                for (int i = 0; i < numuvwtelescopes; i++)
                {
                    getinputline(input, "TELESCOPE");
                ...
                for (int i = 0; i < numscans; i++)
                {
                    ...
                    for (int j = 0; j < configs.length; j++)
                    {
                        if (configs[j].sourcename.trim().equals(sourcename.trim()) &&
                            configs[j].active)
                            sourceactive = true;
                    }
        ...
        public String[] trimDataFiles(String[] datafiles)
        {
            ...
            for (int i = 0; i < datafiles.length; i++)
            {
                ...
                try
                {
                    ...
                    if (bysource)
                    {
                        offset = ( (doy - startdoy) * 86400 +
                                  (hhmmss / 10000) * 3600 +
                                  ( (hhmmss % 10000) / 100) * 60 + hhmmss % 100 -
                                  startseconds) / incrementsecs;
                        keep[i] = active.elementAt(offset).booleanValue();
                    }
                    else if (bymode)
                    {
                        offset = (doy - corrdoy) * 86400 +
                            (hhmmss / 10000) * 3600 +
                            ( (hhmmss % 10000) / 100) * 60 + hhmmss % 100;
                        keep[i] = false;
    ...
    private static class DataFormat
    {
        ...
        public static final String[] formatstrings =
            {
            "LBA", "LBAVSOP", "MKV", "MKV_MKIV", "MKV_VLBA", "K5", "NZ"};
    }
