...
C                          = 'TF' => use IERS 1992 tide model + third order 
C                                    term (tide free crust, pemanent tide
C                                    not removed).
C                          = 'RC' => use IERS 1996 tide model (real crust, 
C                                    permanent tide removed).
C
C   Program specifications:
c     INTEGER*2 NDO(3), KERR
      INTEGER*2      LETDM(40),  New_ON(40),  Old_ON(40), LOFF(40)
      CHARACTER*40 C_LETDM(2), C_New_ON(2), C_Old_ON(2), C_LOFF(2) 
      EQUIVALENCE (C_LETDM,LETDM), (C_New_ON,New_ON), (C_LOFF,LOFF)
      EQUIVALENCE (C_Old_ON, Old_ON)
C
      DATA C_LETDM 
     *   /'Earth Tide Module - IERS 1996 model, las',
     *    't modified 98SEP30, David Gordon, GSFC  '/ 
      DATA C_New_ON                                    ! KETDC=0
     *   /'IERS 1996 Earth Tide Model              ',
     *    '                                        '/
      DATA C_Old_ON                                    ! KETDC=0
     *   /'IERS 1992 Earth Tide Model is being used',
     *    ' instead of IERS 1996 Model             '/
      DATA C_LOFF                                      ! KETDC=1
...
C   PUT the flow control text message depending on KETDC.
      IF (KETDC .EQ. 0 .AND. Use_tide .EQ. 'RC')
     +    CALL PUTA('ETD CFLG      ',New_ON,40,1,1)
      IF (KETDC .EQ. 0 .AND. Use_tide .EQ. 'TF')
     +    CALL PUTA('ETD CFLG      ',Old_ON,40,1,1)
...
C
     ...
     *       dC2LamMoon, dS2LamMoon, dC2LamSun, dS2LamSun
      ...
      Common /TIDE8/ TD82P, TD82V, ELASP, ELASV
C
...
C  Check for which tide model to use at the correlators:
       If (Calc_user .eq. 'C' .and. Use_tide .ne. 'RC') Then
         Go to 1500
       Endif
C
C****************************************************************************
...
C  Permanent tide difference if elastic case used:
       topo17el(1,k) = -.0052D0*topo17(1,k)/h_sub_2
       topo17el(2,k) =   0.D0
       topo17el(3,k) = -.0016D0*topo17(3,k)/l_sub_2
       CALL VECRT( RTTOCF(1,1,k),topo17el(1,k),CF17el(1,k) )
       CALL VECRT( R2000(1,1,1), CF17el(1,k), del17el(1,k) )
C   Combine with previous elastic/anelastic difference
       Do kk = 1,3
        ELASP(kk,k) = ELASP(kk,k) - del17el(kk,k)
       Enddo
C
...
C           Access codes:
C             1. 'ETD CONT' - Database access code for the contributions.
C
C     External input/output - Possible debug output.
C
C     Subroutine Interface:
