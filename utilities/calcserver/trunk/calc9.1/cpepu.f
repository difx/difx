...
C
C*******************************************************************************
      SUBROUTINE PEP ( XJD, CT, TSKIP, EARTH, SUN, XMOON )
      IMPLICIT None
C
C 4.    PEP
C
...
C   Do the PUT's for the Earth, Sun, and Moon coordinates
      CALL PUT4 ('EARTH CE      ', EARTH, 3, 3, 1 )
      CALL PUT4 ('SUN DATA      ', SUN  , 3, 2, 1 )
      CALL PUT4 ('MOONDATA      ', XMOON, 3, 2, 1 )
C
...
C
      RETURN
C
 97   WRITE(6,197) JPL_eph
 197  format(' ERROR: Cannot open JPL_eph file : ',a)
      STOP
C
