...
typedef struct  _STREAM_BLOCK_ {
	...
	//! name of the stream
	/*! for example, "T59_E0_R1_2011_10_17_171150.x3c"*/
	string  strmName;
...
protected:
	...
	static const int mMntBasePntNo = MNT_POINT_BASE_NUMBER;
...
public:
	...
	//! Constructor
	/**
	 * Construct an object of project manager, this constructor is designed
	 * for ESSR since ESSR does not have any disks.
	 */
	Projects();
	...
	/**
	 * Send a "create project" request to lgrMgr, then lgrMgr will create a new
	 * project
	 * @return -1	error on sending message
	 * @return -2   error on ACK
	 * @return 0	success
	 */
//	int pmCreateProject(unsigned diskCount);
	 ...
	 */
	int pmDeleteProject(string prjName);
	...
	int pmGetStreamFiles(STREAM * stream, vector<string>& streamFiles);
...
protected:
	 ...
	 */
	INT64 pmGetProjectDataSize(string projName, vector<int> *missingDisks);
	 ...
	 */
	unsigned long long pmGetStreamSize(STREAM * stream);
};
