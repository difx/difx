...
//    }
	...
	std::cout << inputfiles[n] << std::endl;
    }
    ...
    std::size_t fb_size = sizeof(header); 
    std::cout << "offset = " << offset << std::endl;
    ...
    for (int ii=0;ii<sizeof(header)*8;++ii) {
    ...
    int aa = 0;
    for (int yy=0;yy<deinterlaced_header_bits;++yy) {
    ...
    // Display time code
    int tc_decimal = 0;
    for (int ii=0; ii<time_code_digits.size(); ++ii) {
    ...
    for (int filenum=0; filenum<inputfiles.size(); ++filenum) {
        ...
        long frameheader_bits_di = frameheader_bits/64;
        ...
        for (long long frm=0; frm<numframes; ++frm) {
            ...
            long deinterlaced_framelength = (readlength*8)/ntracks;
            ...
            for (int ii=0;ii<sizeof(frame_buffer)*8;++ii) {
            ...
            for (long yy=0;yy<deinterlaced_framelength;++yy) {
            ...
            //std::exit(0);
            //
            unsigned char frame_buffer_bits_mod[readlength];
          ...
          //     int blah = frame_buffer_bits_mod[xxx] & 0x1;
          //     std::cout << blah;
          // }
          // std::cout << std::endl;
           ...
           int writelength = readlength/8;
           unsigned char frame_buffer_bytes_mod[writelength];
           
           long pp = 0;
           for (long ii = 0; ii<writelength; ++ii) { 
           ...
           // test 
           unsigned char frame_buffer_bits_test[readlength*8];
           for (int ii=0;ii<sizeof(frame_buffer)*8;++ii) {
               frame_buffer_bits_test[ii] = ((1 << (ii % 8)) & (frame_buffer_bytes_mod[ii/8])) >> (ii % 8);
           }
           std::cout << std::endl;
           for (int xxx=0;xxx<1000;++xxx){
               int blah = frame_buffer_bits_test[xxx] & 0x1;
               std::cout << blah;
           }
           std::cout << std::endl;
            ...
            for (int xx=0;xx<ntracks;++xx) {
                for (int yy=0;yy<frameheader_bits_di;++yy) {
                    fh_bits[yy] = deinterlaced_frame[xx][yy];     
                    if (xx == 0){    
                        int blah = fh_bits[yy] & 0x1;   
                        std::cout << blah;
                    } 
                }
               if (xx == 0) {
               ...
               if (0 == xx) {
                   for (int yy=96;yy<148;++yy) {
                    ...
                    for (int ii=0; ii<time_code_digits_1.size(); ++ii) {
