	...
	numsegments)
{
	...
	cinfo << "Opening Streamstor" << endl;
	xlrRC = XLROpen(1, &xlrDevice);
  
  	if(xlrRC == XLR_FAIL)
	{
		XLRClose(xlrDevice);
		cfatal << "Error opening Streamstor device";
		cfatal << "Do you have permission +rw for /dev/windrvr6?" << endl;
		MPI_Abort(MPI_COMM_WORLD, 1);
	}
	else
	{
		cinfo << "Success opening Streamstor device" <<  endl;
	}
	...
	{
		cerror << "Error setting bank mode" << endl;
	}
	...
	if(xlrRC != XLR_SUCCESS)
	{
		cerror << "Error setting data replacement mode / fill pattern" << endl;
	}
...
void NativeMk5DataStream::initialiseFile(int configindex, int fileindex)
{
	...
	{
		doUpdate = 1;
		cinfo << "getting module info" << endl;
	...
	{
		cinfo << "NM5 : continuing read in next scan" << endl;
		do
		{
			scan++;
		...
		{
			cwarn << "No more data on module [" << mpiid << "]" << endl;
			scan = 0;
			dataremaining = false;
			keepreading = false;
#ifdef HAVE_DIFXMESSAGE
		...
		cinfo << "Before[" << mpiid << "] rs = " << readseconds << "  rns = " << readnanoseconds << endl;
		...
		cinfo << "After[" << mpiid << "]  rs = " << readseconds << "  rns = " << readnanoseconds << endl;

		if(readseconds > executeseconds)
		{
			cwarn << "No more data for project on module [" << mpiid << "]" << endl;
			scan = 0;
			dataremaining = false;
			keepreading = false;
#ifdef HAVE_DIFXMESSAGE
	...
	else	/* first time this project */
	{
		...
		cinfo << "NativeMk5DataStream " << mpiid << 
		...
		{
			cerror << "No valid data found - aborting\n";
			dataremaining = false;
			sendMark5Status(MARK5_STATE_NODATA, 0, 0, 0.0, 0.0);
			return;
		}

		cinfo << "Scan info. start = " << scan->start << " off = " << scan->frameoffset << " size = " << scan->framebytes << endl;
	}
	...
	cinfo << "The frame start day is " << scan->mjd << 
	...
	if(doUpdate)
	{
		cinfo << "Updating all configs [" << mpiid << "]" << endl;
	...
	{
		cinfo << "NOT updating all configs [" << mpiid << "]" << endl;
	}

	cinfo << "Scan " << (scan-module.scans) <<" initialised[" << mpiid << "]" << endl;
}
...
void NativeMk5DataStream::moduleToMemory(int buffersegment)
{
	...
	if(start + bytes > scan->start + scan->length)
	{
		...
		cinfo << "[" << mpiid << "] shortening read to only " << bytes << " bytes ";
		cinfo << "(was " << readbytes << ")" << endl;
	}
	...
	for(t = 0; t < 2; t++)
	{
		...
		if(xlrRC != XLR_SUCCESS)
		{
			...
			cerror << "XLRReadImmed returns FAIL." << endl;
			cerror << "Read error at position=" << readpointer << ", length=" << bytes << ", error=" << errStr;
			break;
		}
		...
		for(i = 1; i < 100000; i++)
		{
			...
			else if(xlrRS == XLR_READ_ERROR)
			{
				xlrEC = XLRGetLastError();
				XLRGetErrorMessage(errStr, xlrEC);
				cerror << "NativeMk5 " << mpiid << " XLRReadData error: " << errStr << " position=" << readpointer << " length=" << bytes << endl; 
			...
			if(i % 10000 == 0)
			{
				cinfo << "[" << mpiid << "] Waited " << i << " microsec  state = "; 
		...
		{
			cinfo << "[" << mpiid << "]  XLRClose() being called!" << endl;
			XLRClose(xlrDevice);
			
			cinfo << "[" << mpiid << "]  XLRCardReset() being called!" << endl;
			xlrRC = XLRCardReset(1);
			cinfo << "[" << mpiid << "]  XLRCardReset() called! " << xlrRC << endl;

			cinfo << "[" << mpiid << "]  XLROpen() being called!" << endl;
			xlrRC = XLROpen(1, &xlrDevice);
			cinfo << "[" << mpiid << "]  XLROpen() called! " << xlrRC << endl;
		}
	}

	if(xlrRS != XLR_READ_COMPLETE)
	{
		cerror << "NativeMk5: Waited 10 seconds for a read and gave up.  position=" << readpointer << " length=" << bytes << endl;
		dataremaining = false;
		keepreading = false;
		bufferinfo[buffersegment].validbytes = 0;
		return;
	...
	if(sec != sec2 || fabs(ns - readnanoseconds) > 0.5)
	{
		...
		{
			cerror << invalidtime << "consecutive sync errors starting at readpos " << invalidstart << " (" << sec << "," << "ns) != (" << sec2 << "," << readnanoseconds << ")" << endl ;
		}
...
void NativeMk5DataStream::loopfileread()
{
  ...
  cinfo << "NM5 : loopfileread starting" << endl;
  ...
  if(perr != 0)
    csevere << "NativeMk5DataStream readthread " << mpiid << " error trying to signal main thread to wake up!!!" << endl;
  moduleToMemory(numread++);
  ...
  while((bufferinfo[lastvalidsegment].configindex < 0 || filesread[bufferinfo[lastvalidsegment].configindex] <= confignumfiles[bufferinfo[lastvalidsegment].configindex]) && keepreading)
  {
    ...
    if(keepreading)
    {
      ...
      cinfo << "old config[" << mpiid << "] = " << nextconfigindex << endl;
