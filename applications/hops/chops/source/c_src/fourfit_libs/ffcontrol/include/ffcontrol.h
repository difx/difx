...
int create_fsm();
int criteria_match (struct c_block* cb_ptr, char base[2],
    char sour[31], char group, int time);
int default_cblock (struct c_block *cb_ptr);
char* get_bfstring (char* barg);
void init_tokens(int where);
void free_tokens(int where); /* stub */
...
int parse_control_file (char* control_file_name, char** control_file_buff,
    char** set_string_buff, int where);
int parser(void);
static int append_cblocks (struct c_block **cb_start,
    struct c_block **cb_end, int num);
