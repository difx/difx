...
def main():
    ...
    for bline in baseline_list:
        ...
        filter_list.append( vpal.utility.ContinuousQuantityFilter("snr", args.snr_min, 5000) )
        ff_list = vpal.utility.combined_filter(ff_list_pre, filter_list)

        print("n fringe files after cuts =", str(len(ff_list)))        
        ...
        if len(ff_list) == 0:
            ...
            for ch in args.channels:
                ...
                if len(channel_phase_list[ch]) != 0:
                    ...
                    #print( "(mean, std. dev) phase for channel: ", ch, " =", ''.join(str(np.round(channel_mean_phase[ch],2)).rjust(5)), "+/-", ''.join(str(np.round(channel_stddev[ch],2)).ljust(6)))
