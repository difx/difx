...
def main():
    parser = argparse.ArgumentParser(
        prog='ffres2pcp_mixedmode.py', \
        description='''utility for determining channel-by-channel pc_phases corrections for Legacy/VGOS stations in mixed-mode session''' \
        )

    parser.add_argument('control_file', help='the control file to be applied to all scans')
    parser.add_argument('network_reference_station', help='single character code of the station used as network reference (must be a legacy S/X station)')
    ...
    logging.basicConfig(filename='ffres2pcp-mixedmode' + date_string + '.log', level=loglevel)

    if args.verbosity >= 1:
        print('args: ', args)

    logging.info( 'ffres2pcp_mixedmode program arguments:\n ' + str( args ) )
    ...
    ffres2pcp_conf.mode = "MIXED_MODE"
    ffres2pcp_conf.network_reference_station_pol = 'R'
    ffres2pcp_conf.exp_directory = os.path.abspath(work_dir)
    ffres2pcp_conf.network_reference_station = ref_station
    ffres2pcp_conf.target_stations = target_stations
    ...
    ffres2pcp_conf.dtec_tolerance = 1e6 #does not matter for mixed-mode
    ...
    ffres2pcp_conf.frequency_group = args.frequency_group
    ...
    if args.output_filename != '':
        ...
        if single_scan_mode:
            outputfile_basename = "./cf_" + exp_name + "_" + scan_name + "_" + ref_station + target_stations + "_pcphases"
        else:
            outputfile_basename = "./cf_" + exp_name + "_" + ref_station + target_stations + "_pcphases"
