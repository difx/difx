...
int main (int argc, char *argv[])
	...
	int i, j, k, len, n, nt;
	int nbuff, nlags, nap;
	double a_sin, a_cos, a_amp, a_ph, b_sin, b_cos, b_amp, b_ph;
	...
	if (strlen (argv[1]) != 3)
		{
		(void) printf ("%s Typical usage: \n", me);	/* Nope */
		(void) printf ("    %s  xxx  <  $TMP/cortest  |  more \n", me);
		(void) printf ("    where xxx is the record type to be printed \n");
		(void) printf ("    (that is one of 000, 100, 101, 120, 130, etc.), \n");
		(void) printf ("    and $TMP/cortest is an example of a correlator \n");
		(void) printf ("    data file to be read. \n");
	...
	while (1)
		...
		if ((k = fread ((void *) buff, 1, nbuff, stdin)) != nbuff)
			{			/* Errors */
			(void) printf ("%s:  fread() on stdin returned %d \n", me, k);
			break;
			}			/* Presumably end of file */
		mbuff = k;			/* OK.  Number of bytes read (more later) */
		if (msglev < 1)		/* Debuggery? */
			(void) fprintf (stderr, "%s DEBUG:  Read header type %.3s \n",	/* Yes */
					me, t1->recId);
		/* ** Type-000 record? ** */
		...
		/* ** Type-120 record? ** */
			...
			if (t120->type == COUNTS_PER_LAG)	/* 1 */
				...
				{			/* We can't handle any other type-120 types */
				(void) fprintf (stderr,
						"%s ERROR: \007 Unknown type-120 type %d \n", me,
						t120->type);
				return (-20);	/* Error */
				}
		...
		/* ** Type-203 record? ** */
			...
			{
			nbuff = sizeof (struct type_203) - k;
		...
		/* ** Type-205 record? ** */
			...
			{
			nbuff = sizeof (struct type_205) - k;
		...
		/* ** Type-206 record? ** */
			...
			if (strncmp (t1->recVer, "00", 2) == 0)	/* Version 00? */
				nbuff = sizeof (struct type_206_v0) - k;	/* Yes */
		...
		{
		if (strncmp (t1->recVer, "00", 2) == 0)	/* Version 00? */
			nbuff = sizeof (struct type_207_v0) - k;	/* Yes */
		else			/* Assume version 01 */
		...
		{
		if (strncmp (t1->recVer, "00", 2) == 0)	/* Version 00? */
			nbuff = sizeof (struct type_208_v0) - k;	/* Yes */
		else			/* Assume version 01 */
		...
		/* ** Type-221 record? ** */
			...
			/* Special case variable-length record */
			if (msglev < 1)		/* Debuggery? */
				(void) fprintf (stderr, "%s DEBUG:  ps_length = %d \n",
						me, flip_int(t221->ps_length));
		...
		/* ** Type-309 record? ** */
			...
			{
			nbuff = sizeof (struct type_309) - k;
		...
		/* ** We can't yet handle any other record types ** */
			...
			{			/* Unknown recId */
			(void) fprintf (stderr,
					"%s ERROR: \007 Unknown record type \"%.3s\" \n", me,
					t1->recId);
			return (-21);		/* Error */
			}
		...
		if (mbuff + nbuff > MBUFF)
			...
			(void) fprintf (stderr,	/* Yes */
					"%s ERROR:  Buffer overflow record type %.3s  mbuff+nbuff = %d \n",
					me, t1->recId, mbuff + nbuff);
			return (-22);		/* Fatal error */
			}
		/* ** Read that much more data ** */
		if ((k = fread ((void *) (buff + mbuff), 1, nbuff, stdin)) != nbuff)
			{
			(void) fprintf (stderr, "%s:  fread() on stdin returned %d \n", me,
					k);
			break;
			}			/* Presumably end of file */
		mbuff += k;			/* OK.  Total number of bytes read */
		if (msglev < 1)		/* Debuggery? */
			(void) fprintf (stderr, "%s DEBUG:  Read record type %.3s \n",	/* Yes */
				me, t1->recId);
		...
		if (strncmp (t1->recId, "309", 3) == 0)
			{
			// Mk5B phase cal
			printf ("su %d   ntones %d   ", flip_int(t309->su), flip_int(t309->ntones));
			...
			for (i = 0; i < nt; i++)
				...
				if (i % 4 == 0)
					printf ("\n");
				printf ("%17.3lf  ", flip_double(t309->chan[i].freq));
				}
			printf ("\n");
			for (i = 0; i < nt; i++)
				{
                                if (t309->chan[i].chan_name[0] == 0)
                                    continue;   // this channel is unused, skip it
				printf ("chan_name[%d] = %.8s\n", i, t309->chan[i].chan_name);
				for (j = 0; j < nt; j++)
					{
					printf ("%8x %8x  ", flip_int(t309->chan[i].acc[j][0]),
...
int t2s (			/* Convert a time to a printable string */
	...
	if (arg < 0.0 || arg > 31622400.0)
		...
		if (msglev < 1)		/* Nope.  Debuggery? */
			(void) fprintf (stderr, "%s t2s() ERROR: \007 %.3e seconds is illegal \n",
				me, arg);
