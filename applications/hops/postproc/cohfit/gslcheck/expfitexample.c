//
// nonlinear least squares fitting example from
//   https://www.gnu.org/software/gsl/doc/html/nls.html
// modified to output the final fit module function, see ffun: lines
//
// Amusingly, this is very near the amplitude function form we want to fit.
//
#include <stdlib.h>
#include <stdio.h>
...
// ffun: output final function evaluation
  for (i = 0; i < n; i++) {
      double ti = i * TMAX / (n - 1.0);
      double Yi = FIT(2) + FIT(0) * exp (-FIT(1) * ti);
      double yi = 1.0 + 5 * exp (-1.5 * ti);
      printf ("fact: %g %g\n", ti, yi);
      printf ("ffun: %g %g\n", ti, Yi);
  }
