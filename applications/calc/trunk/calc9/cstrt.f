...
C
       Unit1 = getunit()
C WEW -  replace following line with the next two lines
C      OPEN(Unit1,FILE=CALCON_NAME,STATUS='OLD', IOSTAT= IOS)
       OPEN(Unit1,FILE=CALCON_NAME,STATUS='OLD', IOSTAT= WIOS)
       IOS = WIOS
       IF(IOS.ne.0) then
        WRITE(6,'(
...
C**       print *, ' Using external a prioris  '
         Unit2 = getunit()
C WEW -  replace following line with the next two lines
C       Open(Unit2, File=External_inputs, Status='Old', Iostat=Ios) 
        Open(Unit2, File=External_inputs, Status='Old', Iostat=WIOS)
        Ios = WIOS 
         If(Ios .ne. 0) Then
...
C WEW - add following line
      ...
      Character*4 Utflag
C WEW - following line is superfluous
C     Character*4 Utflag
...
C      OPEN (unit=Iunit, file=Ex_EOP, status='old', Readonly, err=99,
       OPEN (unit=Iunit, file=Ex_EOP, status='old', err=99,
C WEW - replace following line with next two
C    *       Iostat=Ios)
     *       Iostat=WIOS)
       Ios = WIOS
       Go to 101
