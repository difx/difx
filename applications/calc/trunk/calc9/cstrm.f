...
C     GET the star name of the current observation.
      CALL GETA ('STAR ID       ', LSTRNM, 4, 1, 1, NDO, KERR )
      IF ( KERR .EQ. 0 )  GO TO 300
      CALL KILL (6HSTRG  , 1, KERR )
C
...
C     If no match, send a message and quit.
  300 DO 310  NN = 1, NUMSTR
           N = NN
c           write(*,*) LNSTAR(1,N), ", ", LSTRNM(1)
c           write(*,*) LNSTAR(2,N), ", ", LSTRNM(2)
c           write(*,*) LNSTAR(3,N), ", ", LSTRNM(3)
c           write(*,*) LNSTAR(4,N), ", ", LSTRNM(4)
           IF  ( ( LNSTAR(1,N) .EQ. LSTRNM(1) )
     1     .AND. ( LNSTAR(2,N) .EQ. LSTRNM(2) )
     2     .AND. ( LNSTAR(3,N) .EQ. LSTRNM(3) )
     3     .AND. ( LNSTAR(4,N) .EQ. LSTRNM(4) ) )  GO TO 320
  310 CONTINUE
      GO TO 600
C
...
c       Difyrs = ((XJD+UTC) - JDepoch) / 365.25D0
C   Proper motion in RA (convert arc-seconds to time units in radians)
c       PR_RA = P_motion(1,N) / DCOS(DECLINATION) * CONVDS * Difyrs
C   Proper motion in Dec (convert arc-seconds to radians)
c       PR_DEC = P_motion(2,N) * CONVDS * Difyrs
C
...
c       IF (KSTRC.eq.2) THEN
c         RIGHT_ASC   = RIGHT_ASC   + PR_RA
c         DECLINATION = DECLINATION + PR_DEC
c         PR_RA  = -PR_RA 
c         PR_DEC = -PR_DEC
c       ENDIF
C      WRITE(6,8) ' Xepoch, Xdays, XJAN1, JDepoch, Difyrs ', 
C    *              Xepoch, Xdays, XJAN1, JDepoch, Difyrs 
C      WRITE(6,8) ' PR_RA, PR_DEC ', PR_RA, PR_DEC 
C
c     ENDIF
C***********************************************************************
C
...
C                    David Gordon 2000.05.15 Bug correction, variable c1 
C                             redefined (had been commented out).
C
...
C     99.10.27 D. Gordon/GSFC - Extraneous printout removed.
C
      INCLUDE 'cmxsr.i'
      INCLUDE 'inputs.i'
      INCLUDE 'ccon.i'             
...
C  Open the Star catalog data file
       Iunit = getunit()
C WEW - readonly not allowed in G77
...
C WEW replace following line
C1010   Format(4X,A8,2X,2(I2,X),F13.10,1X,A1,2(I2,X),F13.10,3X,A20)
...
C  Non-blokq.dat file
     ...
     *       Psecs, Staref 
C WEW replace following line
C1015   Format(4X,A8,2X,2(I2,X),F13.10,1X,A1,2(I2,X),F13.10,3X,
...
C
 200   Continue
C**    print *,' STRIN: Pmotion, Dpsec = ', Pmotion, Dpsec
C
       Close(Iunit)
C
