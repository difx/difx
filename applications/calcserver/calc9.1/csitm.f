...
C    the case of external file inputs
      If (Input_ocean) Then
        print *, 'ADDs for external ocean loading catalog'
...
C
      ...
      NUMSIT_LOCAL = NUMSIT
C-VLBA Do not retrieve site names, we pass Calc the site index 
C     numbers directly
C      CALL GETA ('SITNAMES      ',LNSITE, 4, NUMSIT_LOCAL, 1, NDO,
C     *            KERR(2) )
       KERR(2) = 0
C
C-----------------------------------------------------------------------------
...
C         write (6,*)"lat, long, h = ",n,xlat(n)*360.0/TWOPI,
C     .   xlon(n)*360.0/TWOPI,height(n)
C
...
C-VLBA Modified to support the VLBA Correlator. Baseline station index
C     numbers are supplied by modlTask.
...
C
C        DO 320  NN = 1, NUMSIT
C          N = NN
C          IF   ( ( LNSITE(1,NN) .EQ. LNBASE(1,L) )
C     1    .AND.  ( LNSITE(2,NN) .EQ. LNBASE(2,L) )
C     2    .AND.  ( LNSITE(3,NN) .EQ. LNBASE(3,L) )
C     3    .AND. ( LNSITE(4,NN) .EQ. LNBASE(4,L) ) )  GO TO 330
C  320   CONTINUE
C
C        GO TO 700
C
...
C  Check for geocenter station
       If(Zero_site .eq. 0) Nzero = 0
       If(Zero_site .ne. 0 .and. N .eq. Zero_site) Then
        Nzero = N
C        print *, ' SITG: Geocenter Site Found, Nzero =  ', Nzero
       Endif
C
...
c     WRITE(6,8)' New DSITP1 ', DSITP(1,1,K), DSITP(2,1,K),DSITP(3,1,K)
c     WRITE(6,8)' New DSITP2 ', DSITP(1,2,K), DSITP(2,2,K),DSITP(3,2,K)
C
...
c       Iunit = getunit()
c       Open (Unit=Iunit, File=Ex_sites, Status='old', Readonly,
...
C  Matched I'th station in data base station list, save station particulars
           II = I
           Read(Inbuf(14:76),*) Sit_X, Sit_Y, Sit_Z, An_typ, Ax_off, 
     *                          S_zen
C
           SITXYZ(1,II) = Sit_X
           SITXYZ(2,II) = Sit_Y
