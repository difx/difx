...
C     Program specifications -
      INTEGER*2      LATMM(40),      LON(40),    LOFF(40),  LWET(40)
      CHARACTER*40 C_LATMM(2) ,    C_LON(2) ,  C_LOFF(2), C_LWET(2)
      EQUIVALENCE (C_LATMM,LATMM),(C_LON,LON),(C_LOFF,LOFF),
     .            (C_LWET,LWET)
C
      DATA C_LATMM /
     .'Atmosphere Module - Last modification 98',
     .'NOV23, D. Gordon, GSFC.                 '/
C
      DATA C_LON /
...
C
      DATA C_LWET /
     .'Atmosphere Module turned on, Niell wet c',
     .'ontributions applied to theoreticals.   '/
C
      DATA C_LOFF /
...
C     PUT module control flag status message according to KATMC.
      IF (KATMC .EQ. 1) THEN
         CALL PUTA('ATM CFLG      ',LON,40,1,1)
         If (Calc_user .eq. 'C' .and. Wet_atm .eq. 'Y') Then
            CALL PUTA('ATM CFLG      ',LWET,40,1,1)
         End If
      END If
...
C      99.10.01  David Gordon Added correlator option to combine dry and 
C                             wet atmosphere corrections.
C
...
C  Store the Niell dry atmosphere delays for use elsewhere:
      ...
      Enddo 
C 
C  Mods for correlator usage. Combine dry and wet contributions and PUT into
C   dry access code. Set wet access code to zero. 
      If (Calc_user .eq. 'C' .and. Wet_atm .eq. 'Y') Then
       Do N=1,2 
C        Datmc_hmf(N,1) = Datmc_hmf(N,1) + Datmc_wmf(N,1)
C        Datmc_hmf(N,2) = Datmc_hmf(N,2) + Datmc_wmf(N,2)
C        Datmc_wmf(N,1) = 0.D0 
C        Datmc_wmf(N,2) = 0.D0 
C  And add wet to dry for use elsewhere 
