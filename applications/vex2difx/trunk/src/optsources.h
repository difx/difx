 ...
 * $HeadURL: https://svn.atnf.csiro.au/difx/applications/vex2difx/trunk/src/optsources.h $
...
#ifndef __SOURCES_STREAM_H__
#define __SOURCES_STREAM_H__
...
class optsources : public std::ofstream
{
public:
	enum PersonalityType {RDBE_UNKNOWN, RDBE_NONE, RDBE_PFB, RDBE_DDC};
	enum RecorderType {RECORDER_NONE, RECORDER_MARK5C};

	optsources(): personalityType(RDBE_UNKNOWN), recorderType(RECORDER_MARK5C), lastValid(0.0), lastSourceId(-1), lastModeId(-1), lastChannelSet(-1), evlaIntSec(0), evlasbChan(0), evlasbBits(0), evlaVCIVersion(0.0), mjd0(0.0), isMark5A(false) {};
