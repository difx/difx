...
% TODO
% Add SniffPlots 
% Add vdifio utilities
% m5access: m5timeseries, m5findformats, m5bstate, findvdif
...
Bug: {\tt cpumon} does not respond nicely to {\tt -h} on the command line.
...
%FIXME: check if new parameters have been added

%FIXME: note other database env vars...
...
\item[] {\tt -d} or {\tt --difx} : run on all {\tt .difx} files found in the directory \difxoneone
\item[] {\tt -k} or {\tt --keep-order} : don't sort the antennas by name \difxoneone
\item[] {\tt -1} or {\tt --dont-combine} : make a separate FITS file for each input job \difxoneone
\item[] {\tt -x} or {\tt --dont-sniff} : don't generate sniffer output files \difxoneone
\item[] {\tt --phasecentre} {\em p} : (U.S.\ spelling OK too) Create a FITS file for all the {\em p}$^{\rm th}$ phase centers (default 0) \difxtwozero
\item[] {\tt --override-version} ignore difx version clashes \difxoneone
\end{itemize}
...
\begin{itemize}
\item[] Usage: {\tt difxcalculator} {\em baseName} $[${\em speedUp}$]$
...
Bug: does not respond nicely to {\tt -h} on command line.

Bug: does not take into consideration multiple phase centers or zoom bands.
...
\begin{itemize}
\item[] Usage: {\tt errormon} $[${\em maxSeverity}$]$
...
Bug: {\tt errormon} and {\tt errormon2} do not respond nicely to {\tt -h} on the command line.
...
\item[] {\tt -h} or {\tt --help} : print usage information and exit
\item[] {\tt -v} or {\tt --verbose} : be more verbose
\item[] {\tt -o} or {\tt --overheadcores} {\em ohc} : leave at least {\em ohc} on each compute node unscheduled \difxoneone
...
% FIXME: does this know about MARK5C/ directories?
...
% FIXME: program should make sure datastream type is MODULE
...
Otherwise the program will run until end of file or interruption by user.
Usage is as follows:
...
See additional documentation under {\tt mk5cp}.
...
\noindent
Program {\tt mk5cp} copies baseband VLBI data from a module to a file somewhere on the unix filesystem, perhaps an external USB disk.
...
Every 10 seconds, {\tt mk5daemon} looks in the {\tt /proc} directory to get information about the CPU load, memory usage, and network traffic.
...
\item[] {\tt -F} or {\tt --fast} : get format details from new-style directory
...
A new option to write a {\tt .dir} file to the module has been introduced in DiFX 2.0.2. 
To use, the VSN has to be written explicitly on the command line (i.e., bank letter cannot be used).
...
The directory will be left in an indeterminate state, so use caution when doing this.
...
Bug: {\tt mk5mon} does not respond nicely to {\tt -h} on the command line.
...
This program is uses parallel computing to make correlation practical on a cluster of ordinary computers.
...
Within the DiFX framework, the user should never have to directly start {\tt mpifxcorr} as this is done more simply with {\tt startdifx} or via the DiFX Operator Interface.
...
\begin{itemize}
\item[] Usage: {\tt statemon} 
\end{itemize}

Bugs: does not respond kindly to a {\tt -h} option.
...
Bugs: does not respond nicely to {\tt -h} on the command line.
...
\noindent
Program {\tt vexpeek} takes a vex file as input and sends to {\tt stdout} the experiment name, segment, and a list of antennas and the MJD times that they were included in the observation.
...
\begin{itemize}
\item[] Usage: {\tt vexpeek} {\em vexFile} 
...
Bug: {\tt vexpeek} does not respond nicely to {\tt -h} on the command line.
